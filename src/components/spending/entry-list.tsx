...
import { RoundUpControl, type GoalOption } from "@/components/spending/round-up-control";
...
export function SpendingEntryList({
  entries,
  goals,
  currency,
}: {
    ...
    <ul className="flex flex-col divide-y divide-border">
      {entries.map((entry) => (
        <li key={entry.id} className="flex flex-col gap-2 py-3">
          ...
          <RoundUpControl spendingLabel={entry.label} amount={entry.amount} goals={goals} currency={currency} />
        </li>
      ))}
    </ul>
  );
